 ...
 * @website: http://www.webrhai.com 欲索取最新版本SliderPlay或是报告Bug，请发送Email至
 *           506713930@qq.com
 * 
...
(function(window, $) {
	$.fn.sliderPlay = function(options) {
		...
		;
		if (sMoveType != 'randomMove' && sMoveType != 'moveTo'
				&& sMoveType != 'opacity') {
			sMoveType = 'moveTo'
		}
		...
		;
		if (bIsShowTitle) {
			this.append('<div class="sliderPlay-title"><span>'
					+ aSliderPlayImg.attr('alt') + '</span></div>');
			var oSliderPlayTitle = this.find('.sliderPlay-title').find('span')
		}
		;
		switch (sMoveType) {
		case 'moveTo':
		...
		function setAttr(index) {
			oSliderPlayTitle
					&& oSliderPlayTitle.html(aSliderPlayImg.eq(index).attr(
							'alt'));
			aSliderPlayBtn
					&& aSliderPlayBtn.removeClass('current').eq(index)
							.addClass('current')
		}
		;
		function setMoveTo() {
			...
			aSliderPlayItem
					.wrapAll('<div class="sliderPlayMain" style="position:absolute;left:0;top:0;'
							+ style + '"></div>');
		...
		function setOptity() {
			...
			move = function() {
				setAttr(iCurIndex);
				aSliderPlayImg.stop().fadeOut(iSpeed).eq(iCurIndex).fadeIn(
						iSpeed);
