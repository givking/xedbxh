...
<link href="<%=basePath%>jspclient/css/cs_global.css" rel="stylesheet" type="text/css" />
<link href="<%=basePath%>jspclient/css/cs_list.css" rel="stylesheet" type="text/css" />
<link href="<%=basePath%>jspclient/css/cs_top.css" rel="stylesheet" type="text/css" />
<link href="<%=basePath%>jspclient/css/cs_foot.css" rel="stylesheet" type="text/css" />
...
<!--列表页主题-->
	<div class="list_main">
		...
		<div class="list_location">
			<span class="t1"><img src="jspclient/image/t_ico.jpg" />
			</span> <span class="t2">当前位置：></span> <span class="t2"><a
				href="webcenter/clienthome">首页></a>
			</span> <span class="t3">${nav.title}</span>
		</div>
		...
		<div class="list_contain">
			...
			<div class="contain_bd">
				...
				<div class="bd_left">
					...
					<div class="l_nav">
						<span
							style="float:left; display:block; margin-top:16px; margin-left:40px; font-size:14px; font-weight:600"><a
							href="${leftnav.menu.url}" style="color:#FFFFFF">${leftnav.menu.title}</a>
						</span>
					</div>
					<c:forEach items="${leftnav.sublist}" var="menu">
						<div class="l_subnav">
							<span
								style="float:left; display:block; margin-top:14px; margin-left:50px;  font-weight:600"><a
				...
				<div class="bd_right">
					...
					<div class="r_top">
						<span
							style="width:5px; height:29px; margin-top:10px; margin-left:20px; float:left; display:block"><img
							src="jspclient/image/menu_list.png" />
						</span><span
							style="margin-top:18px; margin-left:10px; float:left; font-size:16px; font-weight:600; display:block">${nav.title}</span>
					</div>
					...
					<div class="r_list">
						...
						<ul>
							<c:forEach items="${artlist}" var="art">
								<li><span
									style=" width:600px; height:26px; line-height:16px; float:left; display:block"><a
										href="webcenter/webarticlecontent?id=${art.id}&&menu_id=${art.menu_id}">${art.title}</a>
								</span> <span style="line-height:26px; float:right; display:block">${art.createtime}</span>
								</li>
							</c:forEach>
