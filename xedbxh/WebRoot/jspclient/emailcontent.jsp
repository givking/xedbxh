...
<table width="720" height="362" border="0" cellpadding="0" cellspacing="0" class="tb">
  <tr>
    ...
    <td class="tbord"><c:if test="${mvo.mailask.type=='0'}">求助</c:if><c:if test="${mvo.mailask.type=='1'}">建议</c:if><c:if test="${mvo.mailask.type=='2'}">投诉</c:if><c:if test="${mvo.mailask.type=='3'}">咨询</c:if><c:if test="${mvo.mailask.type=='4'}">其他</c:if></td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td colspan="3" class="tbord">${mvo.mailask.title}</td>
  </tr>
  ...
  <c:forEach items="${mvo.unlist}" var="unser">
    ...
    <tr>
    <td height="129" class="left_td tbord">信件回复</td>
