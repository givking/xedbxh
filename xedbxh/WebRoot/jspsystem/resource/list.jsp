...
$(document).ready(function(){
  $(".add").click(function(){
    window.location.href="<%=basePath%>jspsystem/resource/add.jsp";
  });
...
function loadtable(){
$.getJSON("resource/list",{page:1,rows:10,random:Math.random},function(data){
...
laypage({
        ...
        $.getJSON('resource/list', {page:e.curr,rows:10,random:Math.random}, function(res){
...
if(list.length>0){
   ...
   zt="停用";
       ...
       table.append("<tr><td><input onclick='allcheck()' name=\"id\" type=\"checkbox\" value=\""+list[i].id+"\" /></td><td>"+list[i].title+"</td><td>"+list[i].identity+"</td><td>"+list[i].create_time+"</td><td>"+list[i].url+"</td><td><img src="+list[i].icon+" width='60' height='30'/></td><td id='showtype'>"+type+"</td><td>"+list[i].no+"</td><td id='showstate'>"+zt+"</td><td><a href=\"javascript:updateurl('"+list[i].id+"')\" class=\"tablelink\">编辑</a>&nbsp;&nbsp;<a href=\"javascript:del('"+list[i].id+"')\" class=\"tablelink\">删除</a></td></tr>");
...
function del(ids){
$.get("resource/del",{ids:ids},function(data){
var json = eval("("+data+")");
if(json.result){
...
function updateurl(id){
 window.location.href="<%=basePath%>jspsystem/resource/update.jsp?id="+id;
