...
$(document).ready(function(){
  $(".add").click(function(){
    window.location.href="<%=basePath%>jspsystem/webcase/add.jsp";
  });
...
function loadtable(){
$.getJSON("webcase/list",{page:1,rows:10,random:Math.random},function(data){
...
laypage({
        ...
        $.getJSON('webcase/list', {page:e.curr,rows:10,random:Math.random}, function(res){
...
if(list.length>0){
   ...
   zt="停用";
       ...
       table.append("<tr><td><input onclick='allcheck()' name=\"id\" type=\"checkbox\" value=\""+list[i].id+"\" /></td><td>"+list[i].title+"</td><td>"+list[i].casenum+"</td><td>"+list[i].createtime+"</td><td>"+result+"</td><td>"+zt+"</td><td><a href=\"javascript:updateurl('"+list[i].id+"')\" class=\"tablelink\">编辑</a>&nbsp;&nbsp;<a href=\"javascript:del('"+list[i].id+"')\" class=\"tablelink\">删除</a></td></tr>");
...
function del(ids){
$.get("webcase/del",{ids:ids},function(data){
var json = eval("("+data+")");
if(json.result){
...
function updateurl(id){
 window.location.href="<%=basePath%>jspsystem/webcase/update.jsp?id="+id;
}
...
$.getJSON("webcase/list?title$lk="+tit,{page:1,rows:10,random:Math.random},function(data){
...
laypage({
        ...
        curr: 1, //初始化当前页
        jump: function(e){ //触发分页后的回调
        $.getJSON('webcase/list?title$lk='+tit, {page:e.curr,rows:10,random:Math.random}, function(res){
            var indextable=$("#listtable tbody");
            indextable.html("");
          tablerows(res);
          });
