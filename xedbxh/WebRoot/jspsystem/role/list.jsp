...
$(document).ready(function(){
  $(".add").click(function(){
    window.location.href="<%=basePath%>jspsystem/role/add.jsp";
  });
...
function loadtable(){
$.getJSON("role/list",{page:1,rows:10,random:Math.random},function(data){
...
laypage({
        ...
        $.getJSON('role/list', {page:e.curr,rows:10,random:Math.random}, function(res){
...
if(list.length>0){
   ...
   zt="停用";
       ...
       table.append("<tr><td><input onclick='allcheck()' name=\"id\" type=\"checkbox\" value=\""+list[i].id+"\" /></td><td>"+list[i].role_name+"</td><td>"+list[i].create_time+"</td><td>"+zt+"</td><td><a href=\"javascript:updateurl('"+list[i].id+"')\" class=\"tablelink\">编辑</a>&nbsp;&nbsp;<a href=\"javascript:del('"+list[i].id+"')\" class=\"tablelink\">删除</a></td></tr>");
...
function del(ids){
$.get("role/del",{ids:ids},function(data){
var json = eval("("+data+")");
if(json.result){
window.location.href="<%=basePath%>jspsystem/role/list.jsp";
}else{
alert(json.msg);
window.location.href="<%=basePath%>jspsystem/role/list.jsp";
}
...
if(idarr.length>0){
 ...
 layer.open({
        ...
        maxmin: true,
        area: ['180px', '360px'],
        content: ['<%=basePath%>jspsystem/roleres/restree.jsp?id='+idarr[0].value]
...
function updateurl(id){
 window.location.href="<%=basePath%>jspsystem/role/update.jsp?id="+id;
}
...
<body>
    ...
    <div class="tools">
        ...
        <ul class="toolbar1">
        <li onclick="setauth()" style="cursor: pointer;"><span><img src="<%=basePath%>resources/images/t05.png"/></span>设置权限</li>
        </ul>
