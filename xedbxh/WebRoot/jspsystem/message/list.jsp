...
$(document).ready(function(){
  $(".add").click(function(){
    window.location.href="<%=basePath%>jspsystem/download/add.jsp";
  });
...
function loadtable(){
$.getJSON("message/list",{page:1,rows:10,random:Math.random},function(data){
...
laypage({
        ...
        $.getJSON('message/list', {page:e.curr,rows:10,random:Math.random}, function(res){
...
if(list.length>0){
   for(var i = 0;i<list.length;i++){
       table.append("<tr><td><input onclick='allcheck()' name=\"id\" type=\"checkbox\" value=\""+list[i].id+"\" /></td><td>"+list[i].name+"</td><td>"+list[i].phone+"</td><td>"+list[i].email+"</td><td>"+list[i].createtime+"</td><td><a href=\"javascript:updateurl('"+list[i].id+"')\" class=\"tablelink\">查看</a>&nbsp;&nbsp;<a href=\"javascript:del('"+list[i].id+"')\" class=\"tablelink\">删除</a></td></tr>");
...
function del(ids){
$.get("message/delasfile",{ids:ids},function(data){
var json = eval("("+data+")");
if(json.result){
window.location.href="<%=basePath%>jspsystem/message/list.jsp";
}else{
alert(json.msg);
window.location.href="<%=basePath%>jspsystem/message/list.jsp";
}
...
function updateurl(id){
 window.location.href="<%=basePath%>jspsystem/message/update.jsp?id="+id;
}
...
$.getJSON("message/list?title$lk="+tit,{page:1,rows:10,random:Math.random},function(data){
...
laypage({
        ...
        curr: 1, //初始化当前页
        jump: function(e){ //触发分页后的回调
        $.getJSON('message/list?title$lk='+tit, {page:e.curr,rows:10,random:Math.random}, function(res){
            var indextable=$("#listtable tbody");
            indextable.html("");
          tablerows(res);
          });
...
<body>
    ...
    <table id="listtable" class="tablelist">
        ...
        <th>电话</th>
        <th>邮箱</th>
       <%--  <th>留言内容<i class="sort"><img src="<%=basePath%>resources/images/px.gif" /></i></th> --%>
        <th>创建时间</th>
        <th>操作</th>
        </tr>
        </thead>  
