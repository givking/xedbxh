...
$(document).ready(function(){
  $(".add").click(function(){
    window.location.href="<%=basePath%>jspsystem/volunteer/add.jsp";
  });
...
function loadtable(){
$.getJSON("volunteer/list",{page:1,rows:10,random:Math.random},function(data){
...
laypage({
        ...
        $.getJSON('volunteer/list', {page:e.curr,rows:10,random:Math.random}, function(res){
...
if(list.length>0){
   for(var i = 0;i<list.length;i++){
       table.append("<tr><td><input onclick='allcheck()' name=\"id\" type=\"checkbox\" value=\""+list[i].id+"\" /></td><td>"+list[i].volname+"</td><td>"+list[i].volarea+"</td><td>"+list[i].voltime+"</td><td>"+list[i].telephone+"</td><td>"+list[i].createtime+"</td><td><a href=\"javascript:updateurl('"+list[i].id+"')\" class=\"tablelink\">编辑</a>&nbsp;&nbsp;<a href=\"javascript:del('"+list[i].id+"')\" class=\"tablelink\">删除</a></td></tr>");
...
function del(ids){
$.get("volunteer/del",{ids:ids},function(data){
var json = eval("("+data+")");
if(json.result){
...
function updateurl(id){
 window.location.href="<%=basePath%>jspsystem/volunteer/update.jsp?id="+id;
}
...
$.getJSON("volunteer/list?title$lk="+tit,{page:1,rows:10,random:Math.random},function(data){
...
laypage({
        ...
        curr: 1, //初始化当前页
        jump: function(e){ //触发分页后的回调
        $.getJSON('volunteer/list?title$lk='+tit, {page:e.curr,rows:10,random:Math.random}, function(res){
            var indextable=$("#listtable tbody");
            indextable.html("");
          tablerows(res);
          });
...
<body>
    ...
    <table id="listtable" class="tablelist">
        ...
        <th>志愿者姓名<i class="sort"><img src="<%=basePath%>resources/images/px.gif" /></i></th>
        <th>志愿地区</th>
        <th>志愿服务时间</th>
        <th>手机号码</th>
        <th>创建时间</th>
