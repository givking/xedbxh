...
function loadtable(){
$.getJSON("download/list",{page:1,rows:10,random:Math.random},function(data){
...
laypage({
        ...
        $.getJSON('download/list', {page:e.curr,rows:10,random:Math.random}, function(res){
...
if(list.length>0){
   for(var i = 0;i<list.length;i++){
       table.append("<tr><td><input onclick='allcheck()' name=\"id\" type=\"checkbox\" value=\""+list[i].id+"\" /></td><td>"+list[i].filename+"</td><td>"+list[i].create_time+"</td><td><a href=\"javascript:downfile('"+list[i].id+"')\" class=\"tablelink\">下载</a>&nbsp;&nbsp;<a href=\"javascript:del('"+list[i].id+"')\" class=\"tablelink\">删除</a></td></tr>");
...
function del(ids){
$.get("download/delasfile",{ids:ids},function(data){
var json = eval("("+data+")");
if(json.result){
...
/* 不能用ajax  response只能用一次 */
<%-- $.post("download/downfile",{ids:ids},function(data){
var json = eval("("+data+")");
if(json.result){
window.location.href="<%=basePath%>jspsystem/download/list.jsp";
}else{
...
function updateurl(id){
 window.location.href="<%=basePath%>jspsystem/download/update.jsp?id="+id;
}
...
$.getJSON("download/list?title$lk="+tit,{page:1,rows:10,random:Math.random},function(data){
...
laypage({
        ...
        curr: 1, //初始化当前页
        jump: function(e){ //触发分页后的回调
        $.getJSON('download/list?title$lk='+tit, {page:e.curr,rows:10,random:Math.random}, function(res){
            var indextable=$("#listtable tbody");
            indextable.html("");
          tablerows(res);
          });
...
<body>
    ...
    <table id="listtable" class="tablelist">
        ...
        <th>文件名<i class="sort"><img src="<%=basePath%>resources/images/px.gif" /></i></th>
        <th>创建时间</th>
        <th>操作</th>
        </tr>
        </thead>  
